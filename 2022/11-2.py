...
for i in range(monkeys):
    for item in N[i*7+1].split(": ")[1].split(", "):
        items[i].append(item)
    modulosProduct *= int(N[i*7+3].split(" ")[-1])
...
for i in range(10000):
    ...
    for j in range(monkeys):
        for k in range(len(items[j])):
            ...
            modVal = int(N[j*7+3].split(" ")[-1])

            if(items[j][k] % modVal) == 0:
                toMonkey = int(N[j*7+4].split(" ")[-1])
            else:
                toMonkey = int(N[j*7+5].split(" ")[-1])
            items[toMonkey].append(items[j][k] % modulosProduct)
        items[j] = []
